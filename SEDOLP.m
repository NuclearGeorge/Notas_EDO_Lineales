...
(*----------------Elementos gráficos--------------*)
EF[xl_,xr_,yl_,yr_]:=StreamPlot[sis[x,y],{x,xl,xr},{y,yl,yr},ImageSize->500,StreamStyle-> Black,StreamPoints-> Fine,Axes->True,AxesLabel->{Subscript[x, 1],Subscript[x, 2]},LabelStyle->Directive[Black,Bold ,30,FontFamily->"Courier New"],PlotRange-> All,Prolog-> {White,Rectangle[Scaled[{-2,-2}],Scaled[{2,2}]]}];
EFO[xl_,xr_,yl_,yr_,ti_,tf_]:=Show[EF[xl,xr,yl,yr],ParametricPlot[SOLCIX[t],{t,ti,tf},PlotStyle->Red,ImageSize->500,Axes->True,AxesLabel->{Subscript[x, 1],Subscript[x, 2]},LabelStyle->Directive[Black,Bold ,30,FontFamily->"Courier New"],PlotRange-> Automatic,Prolog-> {White,Rectangle[Scaled[{-2,-2}],Scaled[{2,2}]]}]];

ST[ti_,tf_]:=Show[Plot[SOLCIX[t][[1]],{t,ti,tf},PlotStyle->{Blue,Thick},ImageSize->500,AxesLabel->{t," "},LabelStyle->Directive[Black,Bold ,30,FontFamily->"Courier New"],PlotRange-> All,Prolog-> {White,Rectangle[Scaled[{-2,-2}],Scaled[{2,2}]]},RotateLabel->False,FrameTicks->{True,True},Frame-> True,PlotLegends->{"\!\(\*SubscriptBox[\(x\), \(1\)]\)"}],Plot[SOLCIX[t][[2]],{t,ti,tf},PlotStyle->{Green,Thick},ImageSize->500,AxesLabel->{t," "},LabelStyle->Directive[Black,Bold ,30,FontFamily->"Courier New"],PlotRange-> All,Prolog-> {White,Rectangle[Scaled[{-2,-2}],Scaled[{2,2}]]},RotateLabel->False,FrameTicks->{True,True},Frame-> True,PlotLegends->{"\!\(\*SubscriptBox[\(x\), \(2\)]\)"}]];
